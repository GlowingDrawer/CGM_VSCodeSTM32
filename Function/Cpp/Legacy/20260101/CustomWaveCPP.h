...
namespace NS_DAC {
    ...
    // DPV 参数结构体
    struct DPV_Params {
        float startVolt = -0.5f;   // 起始电压
        float endVolt = 0.5f;      // 终止电压
        float stepVolt = 0.004f;   // 阶梯步长 (Step Potential)
        float pulseAmp = 0.05f;    // 脉冲幅度 (Pulse Amplitude)
        float pulseWidth = 0.05f;  // 脉冲宽度 (Pulse Width, e.g., 50ms)
        float pulsePeriod = 0.2f;  // 脉冲周期 (Pulse Period, e.g., 200ms)
        float scanRate = 0.02f;    // 扫描速度 (用于计算等效参数，可选)
    };

    // DPV 运行状态枚举
    enum class DPV_State {
        IDLE,
        WAIT_BASE,  // 处于基电位，等待脉冲开始
        WAIT_PULSE  // 处于脉冲电位，等待脉冲结束
    };
